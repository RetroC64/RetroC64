// Copyright (c) Alexandre Mutel. All rights reserved.
// Licensed under the BSD-Clause 2 license.
// See license.txt file in the project root for full license information.

// Translated from drivecode.s from Spindle3.1
// Original code with MIT license - Copyright (c) 2013-2022 Linus Akesson

// ReSharper disable InconsistentNaming

using AsmMos6502;
using AsmMos6502.Expressions;
using static AsmMos6502.Mos6502Factory;

namespace RetroC64.Loader;

/// <summary>
/// C# translation of drivecode.s for the 1541, using Mos6510Assembler.
/// All comments are preserved from the original source.
/// </summary>
internal class SpinFireDriveCode
{
    // ---------------------- Memory Layout and Constants ----------------------

    // Zero page and memory map
    public const ushort stashbufs = 0x600;
    public const ushort gcrdecode = 0x500;
    public const ushort zonebits = 0x544;
    public const ushort zonebranch = 0x5bd;
    public const ushort zonesectors = 0x5c4;
    public const ushort scramble = 0x51f;
    public const byte SAFETY_MARGIN = 0x07;

    // Zero page variables
    public const byte ZPORG = 0x60; // $66 bytes
    public const byte interested = 0xd8; // 21 + 2 bytes
    public const byte ninterested = 0xef;
    public const byte req_track = 0xf1;
    public const byte currtrack = 0xf2;
    public const byte nextstatus = 0xf3;
    public const byte safety = 0xf4;
    public const byte chunkend = 0xf5;
    public const byte ledmask = 0xf6;
    public const byte temp = 0xf7;
    public const byte chunkprefix = 0xf8;
    public const byte chunklen = 0xf9;
    public const byte bufptr = 0xfa; // word
    public const byte nstashed = 0xfc;
    public const byte tracklength = 0xfd;

    public bool GenerateErrors { get; set; }


    public void Assemble(Mos6510Assembler asm)
    {
        // Spindle by lft, linusakesson.net/software/spindle/
        // This code executes inside the 1541.
        // Memory
        //  000 - Zero page; contains the gcr loop at $30-$91
        //  100 - Stack; used as block buffer
        //  200 - Code for serial communication
        //  300 - Code for fetching data from disk
        //  400 - Miscellaneous code
        //  500 - GCR decoding tables
        //  600 - Init, then stash buffer 1
        //  700 - Stash buffer 2

        // Main entry point at $600
        asm.LabelForward("ondemand_dest", out var ondemand_dest);
        asm.LabelForward("fetch_return", out var fetch_return);
        asm.LabelForward("transfer", out var transfer);
        asm.LabelForward("zpc_entry", out var zpc_entry);
        asm.LabelForward("zpc_bne", out var zpc_bne);
        Mos6502ExpressionU16 BNE_WITH_NOP;
        Mos6502ExpressionU16 BNE_WITHOUT_NOP;
        
        // Flip disk / end code at $600
        asm.LabelForward("ondemand_entry", out var ondemand_entry);

        // Communicate at 0x200
        asm.LabelForward("mod_buf", out var mod_buf);
        asm.LabelForward("checkunit", out var checkunit);
        asm.LabelForward("comm_continue", out var comm_continue);
        asm.LabelForward("err_prob", out var err_prob);

        // Fetch code at $300
        asm.LabelForward("drivecode_fetch", out var drivecode_fetch);
        asm.LabelForward("zp_return", out var zp_return);
        asm.LabelForward("mod_fetchret", out var mod_fetchret);

        // Miscellaneous code at $400 
        asm.LabelForward("ondemand_fetchret", out var ondemand_fetchret);
        asm.LabelForward("sendstash", out var sendstash);
        asm.LabelForward("async_cmd", out var async_cmd);
        asm.LabelForward("nextunit", out var nextunit);
        asm.LabelForward("nothing_fetched", out var nothing_fetched);
        
        AssembleInit(0x600, asm, ondemand_entry, drivecode_fetch, ondemand_dest, fetch_return, mod_fetchret, transfer, zpc_entry, zpc_bne, zp_return, out BNE_WITH_NOP, out BNE_WITHOUT_NOP);
        AssembleMisc(0x400, asm, ondemand_fetchret, ondemand_dest, ondemand_entry, nothing_fetched, sendstash, mod_fetchret, mod_buf, fetch_return, drivecode_fetch, transfer, nextunit, checkunit, comm_continue, async_cmd);
        AssembleFetch(0x300, asm, drivecode_fetch, zpc_bne, nothing_fetched, zpc_entry, zp_return, err_prob, mod_fetchret, ondemand_fetchret, sendstash);
        AssembleCommunication(0x200, asm, BNE_WITH_NOP, BNE_WITHOUT_NOP, err_prob, async_cmd, comm_continue, mod_buf, checkunit, nextunit, drivecode_fetch);
        AssembleFlipDiskEnd(0x600, asm, mod_fetchret, transfer, fetch_return);
        AssembleAsynchronousCommand(0x600, asm, mod_fetchret, drivecode_fetch, fetch_return, transfer);

        asm.End();
    }
    
    private void AssembleInit(ushort address, Mos6510Assembler asm, Mos6502Label ondemand_entry, Mos6502Label drivecode_fetch, Mos6502Label ondemand_dest, Mos6502Label fetch_return, Mos6502Label mod_fetchret, Mos6502Label transfer, Mos6502Label zpc_entry,
        Mos6502Label zpc_bne, Mos6502Label zp_return, out Mos6502ExpressionU16 BNE_WITH_NOP, out Mos6502ExpressionU16 BNE_WITHOUT_NOP)
    {
        asm.Org(address)
            // .byt 12 ; sector number
            .Append(12)
            // ondemand_entry
            .Label(ondemand_entry)
            .LabelForward("init_post_fetch", out var init_post_fetch)
            .JMP(init_post_fetch) // always
                                  // M-E bootstrap jumps to 604
                                  // Load misc, fetch, and decoding table
            .Label("start_init", out var startInit);

        asm.LabelForward("initts", out var initts);

        {
            asm
                .LDX_Imm(5)
                .Label(out var loop)
                .LDA(initts, X)
                .STA(6, X)
                .DEX()
                .BPL(loop);
        }

        asm.LabelForward("dosreadblock", out var dosreadblock);

        {
            asm
                // lda #1 ; Read misc into $400
                .LDA_Imm(1)
                .JSR(dosreadblock)
                // lda #0 ; Read fetch into $300
                .LDA_Imm(0)
                .JSR(dosreadblock)
                // lda #2 ; Read gcr table into $500
                .LDA_Imm(2)
                .JSR(dosreadblock)

                .SEI()

                .LDA_Imm(0x02)
                .STA(0x1800); // Indicate busy
        }
        // Clear all zeropage variables
        {
            asm
                .LDX_Imm(0)
                .TXA()
                .Label(out var loop)
                .STA(0, X)
                .INX()
                .BNE(loop);
        }

        asm
            // Read mode, SO enabled
            .LDA_Imm(0xee)
            .STA(0x1c0c)

            // ldx #0
            .STX(0x1c04) // latch, low byte
            .LDA_Imm(0x7f) // disable interrupts
            .STA(0x1c0e)
            .STA(0x1c0d)
            .LDA_Imm(0xc0) // enable timer 1 interrupt
            .STA(0x1c0e)
            .INX()
            .STX(0x1c0b) // timer 1 one-shot, latch port a
            .STX(0x1c05) // quick first timeout

            // Copy zp code into place
            .LabelForward("zpcodeblock_to_relocate", out var zpcodeblock_to_relocate)
            .LabelForward("zpcodeblock_begin", out var zpcodeblock_begin)
            .LabelForward("zpcodeblock_end", out var zpcodeblock_end);

        var zpcodeblock_length = ((zpcodeblock_end - zpcodeblock_begin - 1).LowByte());

        asm
            .LDX_Imm(zpcodeblock_length)
            .Label("zpcode_copy_loop", out var zpcodeCopyLoop)
            .LDA(zpcodeblock_to_relocate, X)
            .STA(ZPORG, X)
            .DEX()
            .BPL(zpcodeCopyLoop)

            // Load the communication code using the newly installed drivecode.
            .LDA_Imm(0x0c)
            .STA(ledmask)
            .LDA(0x1c00)
            .AND_Imm(0x03)
            .ORA_Imm(0x4c) // led and motor on, bitrate for track 18
            .STA(0x1c00)

            .LDA_Imm(19)
            .STA(tracklength)

            .LDX_Imm(18 * 2)
            .STX(currtrack)
            .STX(req_track)

            .INC(interested + 17)
            .INC(ninterested)
            .JMP(drivecode_fetch);

        asm.Label(init_post_fetch)
            .LDA_Imm(6)
            .STA(ondemand_dest + 2)
            .Label(out var ll)
            .BIT(0x1800) // Wait until the host is up (atn held)
            .BPL(ll);

        // Initialisation complete.
        asm.LDA_Imm(1 * 2)
            .STA(req_track)
            .LDA_Imm(fetch_return.LowByte())
            .STA(mod_fetchret + 1)

            // We have the first continuation record
            // ready in the sector buffer.

            .JMP(transfer);

        asm.Label(dosreadblock)
            .STA(0xf9)
            .JMP(0xd586);

        asm.Label(initts)
            .AppendBuffer([18, 11, 18, 2, 18, 3]);

        asm.Label(zpcodeblock_to_relocate)
            .Org(ZPORG)
            .Label(zpcodeblock_begin)
            .Label("zpc_loop", out var zpc_loop)
            // This nop is needed for the slow bitrates (at least for 00),
            // because apparently the third byte after a bvc sync might not be
            // ready at cycle 65 after all.
            // 
            // However, with the nop, the best case time for the entire loop
            // is 128 cycles, which leaves too little slack for motor speed
            // variance at bitrate 11.
            // 
            // Thus, we modify the bne instruction at the end of the loop to
            // either include or skip the nop depending on the current
            // bitrate.
            .NOP()
            .Label("zpc_loop", out var zpc_loop_without_nop)
            .LAX(0x1c01) // lax1c01
            .ARR_Imm(0xf0) // arr imm, ddddd000
            .CLV()
            .TAY()
            .Label("zpc_mod3", out var zpc_mod3)
            .LDA(gcrdecode)
            .ORA(gcrdecode + 1, Y)

            .Label(out var llZp1)
            .BVC(llZp1)

            .PHA()
            .Label(zpc_entry)
            .LDA_Imm(0x0f)
            .LabelForward("zpc_mod5", out var zpc_mod5)
            .SAX((zpc_mod5 + 1).LowByte())
            .LDA(0x1c01)
            .LDX_Imm(3)
            .LabelForward("zpc_mod7", out var zpc_mod7)
            .SAX((zpc_mod7 + 1).LowByte())
            .ALR_Imm(0xfc)
            .TAY()
            .LDX_Imm(0x79)
            .Label(zpc_mod5)
            .LDA(gcrdecode, X)
            .EOR(gcrdecode + 0x40, Y)
            .PHA()
            .LAX(0x1c01)
            .CLV()
            .AND_Imm(0x1f)
            .TAY()

            .Label(out var llZp2)
            .BVC(llZp2)

            .LDA_Imm(0xe0)
            .SBX_Imm(0x00)
            .Label(zpc_mod7)
            .LDA(gcrdecode, X)
            .ORA(gcrdecode + 0x20, Y)
            .PHA()

            // start of a new 5-byte chunk

            .LDA(0x1c01)
            .LDX_Imm(0xf8)
            .LabelForward("zpc_mod1", out var zpc_mod1)
            .SAX((zpc_mod1 + 1).LowByte())
            .AND_Imm(0x07)
            .ORA_Imm(0x08)
            .TAY()

            .LDA(0x1c01)
            .LDX_Imm(0xc0)
            .LabelForward("zpc_mod2", out var zpc_mod2)
            .SAX((zpc_mod2 + 1).LowByte())
            .ALR_Imm(0x3f)
            .STA((zpc_mod3 + 1).LowByte())
            .Label(zpc_mod1)
            .LDA(gcrdecode)
            .Label(zpc_mod2)
            .EOR(gcrdecode, Y)
            .PHA()
            .TSX()
            .Label(zpc_bne);
        BNE_WITH_NOP = (zpc_loop - (zpc_bne + 2));
        BNE_WITHOUT_NOP = BNE_WITH_NOP + 1;
        asm
            .BNE(zpc_loop_without_nop)
            .LDX((zpc_mod3 + 1).LowByte())
            .LDA(0x1c01)
            .JMP(zp_return)
            .Label(zpcodeblock_end);

        asm.AppendBytes(256 - asm.SizeInBytes, 0xaa); // Cannot use CurrentOffset as the BaseAddress was reset for zp
    }

    private void AssembleMisc(ushort address, Mos6510Assembler asm, Mos6502Label ondemand_fetchret, Mos6502Label ondemand_dest, Mos6502Label ondemand_entry, Mos6502Label nothing_fetched, Mos6502Label sendstash, Mos6502Label mod_fetchret, Mos6502Label mod_buf,
        Mos6502Label fetch_return, Mos6502Label drivecode_fetch, Mos6502Label transfer, Mos6502Label nextunit, Mos6502Label checkunit, Mos6502Label comm_continue, Mos6502Label async_cmd)
    {
        asm.Org(address)
            // The new drivecode is now on the stack page; move it.
            .Label(ondemand_fetchret)

            .LDX_Imm(0)

            .Label("ondemand_move_loop", out var ondemandMoveLoop)

            .LDA(0x100, X) // TODO: use label?

            .Label(ondemand_dest)
            .STA(0x200, X) // TODO: use label?
            .INX()
            .BNE(ondemandMoveLoop)

            .JMP(ondemand_entry)

            // nothing_fetched
            .Label(nothing_fetched)

            // More sectors in the current batch?
            .LabelForward("fetchmore", out var fetchmore)
            .LDA(ninterested)
            .BNE(fetchmore)

            // Nothing new to fetch. Is the next batch on a new track?
            .LabelForward("nonewtrack", out var nonewtrack)
            .LDA_Imm(0x40)
            .BIT(2)
            .BEQ(nonewtrack)

            .EOR(2)
            .STA(2)

            .LabelForward("not18", out var not18)
            .LDX(req_track)
            .INX()
            .INX()
            .CPX_Imm(18 * 2)
            .BNE(not18)

            .LDX_Imm(19 * 2)

            .Label(not18)
            .STX(req_track)
            .BNE(fetchmore)

            .Label(nonewtrack)

            // Do we have a stashed sector?
            .LabelForward("nostash", out var nostash)
            .LDX(nstashed)
            .BEQ(nostash)

            .LabelForward("transferbuf", out var transferbuf)
            .Label(sendstash)
            .DEX()
            .STX(nstashed)
            .TXA()
            .ORA_Imm((byte)(stashbufs >> 8))
            .JMP(transferbuf)

            .Label(nostash)

            // Unpack the continuation buffer.
            .LDY_Imm(0)
            .LDX_Imm(20)
            .SEC()

            .Label("newbyte", out var newbyte)
            .LDA(0, Y)
            .INY()

            .Label("bitloop", out var bitloop)
            .LabelForward("notset", out var notset)
            .ROR()
            .BCC(notset)

            .BEQ(newbyte)

            .INC(interested, X)
            .INC(ninterested)
            .CLC()

            .Label(notset)
            .DEX()
            .BPL(bitloop)

            .LabelForward("nodemand", out var nodemand)
            .LSR()
            .BCC(nodemand)

            .LDA_Imm(ondemand_fetchret.LowByte())
            .STA(mod_fetchret + 1)
            .LDA_Imm(18 * 2)
            .STA(req_track)

            .LabelForward("checkunit0", out var checkunit0)
            .Label(nodemand)
            .LDA_Imm(0)
            .STA(bufptr + 1)
            .STA(mod_buf + 2)
            .BEQ(checkunit0)

            .Label(fetch_return)
            .LabelForward("dontstash", out var dontstash)
            // We have a valid sector in the buffer.
            // x is the sector number and z is set if
            // ninterested was decremented to zero.
            .BEQ(dontstash)

            .LDY(nstashed)
            .CPY_Imm(2)
            .BCS(dontstash)

            .LabelForward("dostash", out var dostash)
            .LDA(interested + 2, X)
            .BNE(dostash)

            .LDA_Imm(0x04)
            .AND(0x1800)
            .BNE(dontstash)

            .Label(dostash)
            // Stash this sector, then go fetch another one.

            .LabelForward("mod_dest", out var mod_dest)
            .TYA()
            .ORA_Imm((byte)(stashbufs >> 8))
            .STA(mod_dest + 2)

            .LDX_Imm(0)

            .Label("stash_loop", out var stashLoop)
            .LDA(0x100, X)
            .Label(mod_dest)
            .STA(stashbufs, X)
            .DEX()
            .BNE(stashLoop)

            .INC(nstashed)
            .Label(fetchmore)
            .JMP(drivecode_fetch)

            .Label(dontstash)

            .Label(transfer)
            // Turn off LED.

            .LDA(0x1c00)
            .AND_Imm(0x77)
            .STA(0x1c00)

            .LDA_Imm(1)
            .Label(transferbuf)
            .STA(bufptr + 1)
            .STA(mod_buf + 2)

            // Full block?

            .LabelForward("notfull", out var notfull)
            .LDY_Imm(0)
            .LDA(_[bufptr], Y)
            .BPL(notfull)

            .LDA_Imm(0xff)
            .BMI(nextunit)

            .Label(notfull)
            .LabelForward("nocontrec", out var nocontrec)
            .INY()
            .ASL()
            .BPL(nocontrec)

            .LDX_Imm(0)

            // A continuation record begins with a 3-byte
            // sector set that can be followed by one or more
            // postponed units of size 2..4.

            .LDA_Imm(2)
            .Label("headloop", out var headloop)
            .STA(temp)
            .Label("copy", out var copy)
            .LDA(_[bufptr], Y)
            .INY()
            .STA(0, X)
            .INX()
            .DEC(temp)
            .BPL(copy)

            .LabelForward("headdone", out var headdone)
            .LDA(_[bufptr], Y)
            .BEQ(headdone)

            .CMP_Imm(5)
            .BCC(headloop)

            .Label(headdone)

            .LabelForward("neednodummy", out var neednodummy)
            .CPX_Imm(3)
            .BNE(neednodummy)

            .TXA()
            .STA(0, X)
            .LDX_Imm(7)
            .Label(neednodummy)
            .LDA_Imm(0)
            .STA(0, X)
            .Label(nocontrec)

            .Label(checkunit0)
            .JMP(checkunit)

            .Label(nextunit)
            .STA(chunklen)
            .LDX_Imm(0x09)
            .SBX_Imm(0x00)
            .EOR(scramble, X)
            .STA(chunkprefix)
            .JMP(comm_continue)

            .Label(async_cmd)

            .STA(0x1800)
            .LDX_Imm(23)
            .LDA_Imm(0)

            .Label("async_clear_loop", out var asyncClearLoop)
            .STA(interested, X)
            .DEX()
            .BPL(asyncClearLoop)

            .STA(nstashed)

            // Fetch command-handling code and jump to it.

            .INC(interested + 6)
            .INC(ninterested)
            .LDA_Imm(ondemand_fetchret.LowByte())
            .STA(mod_fetchret + 1)
            .LDA_Imm(ondemand_fetchret.HighByte())
            .STA(mod_fetchret + 2)
            .LDA_Imm(18 * 2)
            .STA(req_track)
            .BNE(fetchmore) // always

            .AppendBytes(256 - asm.CurrentOffset, 0xbb);
    }


    private void AssembleFetch(ushort address, Mos6510Assembler asm, Mos6502Label drivecode_fetch, Mos6502Label zpc_bne, Mos6502Label nothing_fetched, Mos6502Label zpc_entry, Mos6502Label zp_return, Mos6502Label err_prob, Mos6502Label mod_fetchret,
        Mos6502Label ondemand_fetchret, Mos6502Label sendstash)
    {
        asm.Org(address)
            .Label(drivecode_fetch)
            // Fetch code for reading from disk and GCR decoding
            // Wait for previous step to settle
            .Label("wait", out var waitLabel)
            .BIT(0x1c0d)
            .BPL(waitLabel)

            .LDA(0x1c00)
            .LDX(req_track)
            .CPX(currtrack)
            .LabelForward("fetch_here", out var fetch_here)
            .BEQ(fetch_here)

            .AND_Imm(0x0b) // clear zone and motor bits for now
            .LabelForward("seek_up", out var seek_up)
            .BCS(seek_up)

            .Label("seek_down", out var seekDown)
            .DEC(currtrack)
            .ADC_Imm(3) // bits should decrease
            .LabelForward("do_seek", out var do_seek)
            .BCC(do_seek)

            .Label(seek_up)
            .INC(currtrack)
            .ADC_Imm(0) // bits should increase (adc #$01-1)

            .Label(do_seek)
            .LDY_Imm(3)
            .CPX_Imm(31 * 2)
            .LabelForward("ratedone", out var ratedone)
            .BCS(ratedone)
            .DEY()
            .CPX_Imm(25 * 2)
            .BCS(ratedone)
            .DEY()
            .CPX_Imm(18 * 2)
            .BCS(ratedone)
            .DEY()

            .Label(ratedone)
            .LDX(zonebranch, Y)
            .STX((zpc_bne + 1).LowByte())

            .LDX(zonesectors, Y)
            .STX(tracklength)

            .ORA(zonebits, Y) // also turn on motor and LED
            .STA(0x1c00)

            .LDY_Imm(0x19)
            .STY(0x1c05) // write latch & counter, clear int

            .Label("nosectors", out var nosectors)
            .JMP(nothing_fetched)

            .Label(fetch_here)
            .LDX(ninterested)
            .BEQ(nosectors)
            .ORA(ledmask) // turn on motor and usually LED
            .STA(0x1c00)

            .Label("fetchblock", out var fetchblock)
            .LDX_Imm((byte)Mos6510OpCode.BEQ_Relative)
            .LDA(safety)
            .LabelForward("nosaf", out var nosaf)
            .BEQ(nosaf)

            .LDX_Imm((byte)Mos6510OpCode.LDA_Immediate)

            .Label(nosaf)
            .LabelForward("mod_safety", out var mod_safety)
            .STX(mod_safety)

            .Label("prof_sync", out var prof_sync)
            .LDX_Imm(0)
            .TXS()

            .Label("waitsync", out var waitsync)
            .BIT(0x1c00)
            .BPL(waitsync)

            .Label("waitsync2", out var waitsync2)
            .BIT(0x1c00)
            .BMI(waitsync2)

            .LDA(0x1c01) // ack the sync byte
            .CLV()

            .BVC(-2)

            .LDA(0x1c01) // aaaaabbb, which is 01010.010(01) for a header
            .CLV()
            .EOR_Imm(0x55)
            .BNE(waitsync)

            .BVC(-2)
            .LDA(0x1c01) // bbcccccd
            .CLV()
            .ALR_Imm(0x3f)
            .LabelForward("first_mod3", out var first_mod3)
            .STA(first_mod3 + 1)

            .BVC(-2)
            .LAX(0x1c01)
            .ARR_Imm(0xf0)
            .CLV()
            .TAY()

            .Label(first_mod3)
            .LDA(gcrdecode)
            .ORA(gcrdecode + 1, Y)
            .PHA()

            // get sector number from the lowest 5 bits of the first byte

            .AND_Imm(0x1f)
            .TAY()
            .LDA(interested, Y)

            .Label(mod_safety)
            .LabelForward("notint", out var notint)
            .BEQ(notint)

            .JMP(zpc_entry)

            .Label(zp_return)
            .ARR_Imm(0xf0)
            .TAY()
            .LDA(gcrdecode, X)
            .ORA(gcrdecode + 1, Y)

            .Label("prof_sum", out var prof_sum)
            .LabelForward("badsum", out var badsum);

        if (GenerateErrors)
        {
            asm.LDX_Imm(0x3f)
                .Label("loop", out var loop)
                .EOR(0x100, X)
                .EOR(0x140, X)
                .EOR(0x180, X)
                .EOR(0x1c0, X)
                .DEX()
                .BPL(loop)

                .TAX()
                .BNE(badsum)

                .LDA(0x1804) // timer low-byte
                .CMP(err_prob)
                .BCC(badsum);
        }
        else
        {
            asm.LDX_Imm(0x1f)
                .Label("loop", out var loop)
                .EOR(0x100, X)
                .EOR(0x120, X)
                .EOR(0x140, X)
                .EOR(0x160, X)
                .EOR(0x180, X)
                .EOR(0x1a0, X)
                .EOR(0x1c0, X)
                .EOR(0x1e0, X)
                .DEX()
                .BPL(loop)

                .TAX()
                .BNE(badsum);
        }

        {
            asm
                .LSR(safety)
                .BCS(badsum)

                .LDA(0x100)
                .AND_Imm(0x1f)
                .TAX()
                .LSR(interested, X)
                .DEC(ninterested)

                .Label(mod_fetchret)
                .JMP(ondemand_fetchret) // usually jmp fetch_return

                .Label(notint)

                // Are we keeping up with the Commodore ?
                // The block under the drive head isn't interesting.
                // So if no interesting block is about to arrive...

                .LDX_Imm(2)

                .Label("loop", out var loop)
                .LabelForward("nowrap", out var nowrap)
                .INY()
                .CPY(tracklength)
                .BCC(nowrap)

                .LDY_Imm(0)

                .Label(nowrap)
                .LabelForward("fetchblock0", out var fetchblock0)
                .LDA(interested, Y)
                .BNE(fetchblock0)

                .DEX()
                .BNE(loop)

                // ...and we have a stashed sector...
                .LDX(nstashed)
                .BEQ(fetchblock0)

                // ...and the host has nothing better to do...
                .LDA_Imm(4)
                .BIT(0x1800)
                .BEQ(fetchblock0)

                // ...then now is a good time to transmit it.
                .JMP(sendstash)
                .Label(fetchblock0)

                .Label(badsum)
                .JMP(fetchblock)

                .AppendBytes(256 - asm.CurrentOffset, 0xcc);
        }
    }

    private void AssembleCommunication(ushort address, Mos6510Assembler asm, Mos6502ExpressionU16 BNE_WITH_NOP, Mos6502ExpressionU16 BNE_WITHOUT_NOP, Mos6502Label err_prob, Mos6502Label async_cmd, Mos6502Label comm_continue, Mos6502Label mod_buf,
        Mos6502Label checkunit, Mos6502Label nextunit, Mos6502Label drivecode_fetch)
    {
        asm.Org(address)
            // sector number + cont rec
            .Append((byte)(17 | 0x40))
            // These are modified when the disk image is created.
            .AppendBytes(3, 0)
            .Append(0) // No regular units
            .Label("sideid", out var sideid)
            // Knock code for this disk side.
            // Putting the branch offsets here is a kludge to bring them
            // to disk.c so they can be included in the gcr table.
            .Append(0).Append(BNE_WITH_NOP.LowByte()).Append(BNE_WITHOUT_NOP.LowByte())

            .Label(err_prob)
            .Append(0) // Error probability

            .Label("prof_comm", out var prof_comm)

            .Label("nodatarequest", out var nodatarequest)
            .LabelForward("reset", out var reset)
            .BEQ(reset)

            .JMP(async_cmd)

            .Label(reset)
            // Atn was released but no other line is held.
            .JMP(_[0xfffc]) // System reset detected -- reset drive.

            .Label(comm_continue)
            .STY(temp)
            .TYA()
            .CLC()
            .ADC(chunklen)
            .TAY()
            .STY(chunkend)

            .LDX_Imm(2) // MORE

            .LabelForward("notlast", out var notlast)
            .LDA(bufptr + 1) // are we transferring postponed units?
            .BNE(notlast)

            // Wait for the host to pull clock before we
            // can transmit a postponed unit.

            .LDA_Imm(4)
            .Label("waitclk", out var waitclk)
            .BIT(0x1800)
            .BEQ(waitclk)

            .LabelForward("nochain", out var nochain)
            .CPX(chunklen) // is it a chain head?
            .BNE(nochain)

            .LDX_Imm(0xa) // MORE + force chain

            .Label(nochain)
            // y points to last byte of unit

            .LDA(1, Y) // more postponed units to follow?
            .BNE(notlast)

            .BIT(2) // check newjob flag of next batch
            .BPL(notlast)

            .DEX() // turn off MORE, keep force-chain bit
            .DEX()

            .Label(notlast)
            .LDA(nextstatus) // bit 1 set if the old job continues
            .STX(nextstatus)

            .AND_Imm(2)
            .LabelForward("keepmotor", out var keepmotor)
            .BNE(keepmotor)

            // We are about to wait for the host at a job boundary.
            // If the host doesn't pull clock within one second,
            // we'll turn off the motor (and LED).

            .TAX() // x = 0
            .Label("outermotor", out var outermotor)
            .LDA_Imm(0x9e)
            .STA(0x1805)
            .LDA_Imm(4)
            .Label("innermotor", out var innermotor)
            .BIT(0x1800)
            .BNE(keepmotor)

            .BIT(0x1805)
            .BMI(innermotor)

            .INX()
            .BPL(outermotor)

            .LDA(0x1c00)
            .AND_Imm(0xf3)
            .STA(0x1c00)
            .LDA_Imm(SAFETY_MARGIN)
            .STA(safety)

            .Label(keepmotor)
            // Was atn released prematurely? Then reset.

            .LDA(0x1800)
            .BPL(reset)

            .LDA_Imm(0x10)
            .STA(0x1800) // release BUSY (data)

            .BIT(0x1800)
            .BMI(-5) // wait for atn to be released

            .Label("prof_send", out var prof_send)
            .LDX_Imm(0)
            .STX(0x1800) // prepare to read data/clock lines
            .LDY(temp)

            .LDA(0x1800)
            .ALR_Imm(5)
            .BCC(nodatarequest)

            .LabelForward("readyforchain", out var readyforchain)
            .BNE(readyforchain) // host did pull clock

            // The host is currently dealing with a chain, so we
            // have to pull clock as part of the exit status flags
            // (it gets inverted in transmission).

            .LDA(nextstatus)
            .ORA_Imm(8)
            .STA(nextstatus)

            // In this case, we know the motor is already running.

            .LabelForward("motorok", out var motorok)
            .BNE(motorok) // always

            .Label(readyforchain)

            // This could be the first transfer of a new job.
            // Warm up the motor for the next block.

            .LDA(0x1c00)
            .ORA_Imm(4)
            .STA(0x1c00)

            .Label(motorok)

            // Send chunkprefix, then from y+1 to chunkend inclusive, then nextstatus.

            // First bit pair is on the bus 51 cycles after atn edge, worst case.

            .LAX(chunkprefix)
            .AND_Imm(0x0f)
            .LabelForward("sendentry", out var sendentry)
            .BPL(sendentry)

            // Writes to $1800 must happen 13 cycles after atn changed, worst case.
            // Atn can change 4 cycles after writing to $1800, worst case.
            // This allows up to 7 cycles between each check+write idiom.
            .Label("sendloop", out var sendloop)
            .ALR_Imm(0xf0)

            .BIT(0x1800)
            .BMI(-5)
            .STA(0x1800) // 0--cd000

            .LSR()
            .ALR_Imm(0xf0)
            .INY()

            .BIT(0x1800)
            .BPL(-5)
            .STA(0x1800) // 000ab000 (a gets inverted due to atna)

            .Label(mod_buf)
            .LAX(0x100, Y)
            .AND_Imm(0x0f)

            .BIT(0x1800)
            .BMI(-5)

            .Label(sendentry)
            .STA(0x1800) // 0000e-g-
            .ASL()
            .ORA_Imm(0x10)
            .CPY(chunkend)
            .BIT(0x1800)
            .BPL(-5)
            .STA(0x1800) // 0001f-h0
            .TXA()
            .BCC(sendloop)

            .ALR_Imm(0xf0)
            .BIT(0x1800)
            .BMI(-5)
            .STA(0x1800) // 0--cd000 (final byte)

            .LSR()
            .ALR_Imm(0xf0)

            .BIT(0x1800)
            .BPL(-5)
            .STA(0x1800) // 000ab000 (final byte, a gets inverted)

            .LDA(nextstatus)

            .BIT(0x1800)
            .BMI(-5)
            .STA(0x1800) // send status bits

            .LDA_Imm(2)

            .BIT(0x1800)
            .BPL(-5)
            .STA(0x1800) // pull data (BUSY), release the clock line

            .INY()
            .LabelForward("nomoreunits", out var nomoreunits)
            .BEQ(nomoreunits)

            .Label(checkunit)
            .LDA(_[bufptr], Y)
            .BEQ(nomoreunits)
            .JMP(nextunit)

            .Label(nomoreunits)
            .JMP(drivecode_fetch);

        asm.AppendBytes(256 - asm.CurrentOffset, 0xdd);
    }

    private static void AssembleFlipDiskEnd(ushort address, Mos6510Assembler asm, Mos6502Label mod_fetchret, Mos6502Label transfer, Mos6502Label fetch_return)
    {
        asm.Org(address)
            .Append(5) // sector number

            // We have transmitted the chain heads for the last job on the
            // old disk, and the host has returned from the last regular
            // loadercall. Then we've stepped to track 18 and retrieved
            // this code and jumped to it.

            .LabelForward("flip_fetchret", out var flip_fetchret)
            .LDA_Imm(flip_fetchret.LowByte())
            .STA(mod_fetchret + 1)
            .LDA_Imm(flip_fetchret.HighByte())
            .STA(mod_fetchret + 2)
            .LDA_Imm(0x04)
            .STA(ledmask)

            .Label("badflip", out var badflip)
            // Replace the buffer with instructions to transfer a do-nothing
            // unit and then read sector 17 (with the initial continuation
            // record).

            // If the host isn't ready, we'll turn off the motor and wait
            // for the dummy transfer.

            // When the extra "flip disk" loadercall happens, we proceed to
            // alternate between transferring do-nothing units and reading
            // sector 17 again, until the knock codes match or the host
            // intervenes with a command or system reset.

            .LDY_Imm(10)
            .Label("copyretry", out var copyretry)
            .LabelForward("retrysector", out var retrysector)
            .LDA(retrysector, Y)
            .STA(0x100, Y)
            .DEY()
            .BPL(copyretry)
            .JMP(transfer)

            .Label(flip_fetchret)
            // Do the knock codes match?
            .LDX_Imm(2)
            .Label("flipcheck", out var flipcheck)
            .LDA(0x105, X)
            .LabelForward("nextsideid", out var nextsideid)
            .CMP(nextsideid, X)
            .BNE(badflip)

            .DEX()
            .BPL(flipcheck)

            // Yes, this is the new diskside.

            // Set the new-job flag for the initial continuation record.
            // This makes the "flip disk" loadercall return.

            .ASL(0x103)
            .SEC()
            .ROR(0x103)

            .LDA_Imm(0x0c)
            .STA(ledmask)
            .LDA_Imm(1 * 2)
            .STA(req_track)
            .LDA_Imm(fetch_return.LowByte())
            .STA(mod_fetchret + 1)
            .LDA_Imm(fetch_return.HighByte())
            .STA(mod_fetchret + 2)

            // We have a continuation record ready in the sector buffer.

            .JMP(transfer)

            .AppendBytes(256 - 14 - asm.CurrentOffset, 0xee)

            .Label(retrysector)
            .Append(0x40) // Continuation record indicator
            .AppendBuffer([8, 0, 0]) // Continue with sector 17
            .AppendBuffer([5, 0, 0xbf, 0, 0x8f, 0]) // Dummy data unit(patched in disk.c)
            .Append(0) // No more units

            .Label(nextsideid).AppendBytes(3, 0); // Patched
    }
    private void AssembleAsynchronousCommand(ushort address, Mos6510Assembler asm, Mos6502Label mod_fetchret, Mos6502Label drivecode_fetch, Mos6502Label fetch_return, Mos6502Label transfer)
    {
        asm.Org(address)
            .Append(6) // sector number

            .LDY_Imm(0)
            .LDA_Imm(2)
            .STA(temp)
            .ASL() // a = 4 for bit-check

            .Label("bitloop", out var bitloop)
            .BIT(0x1800)
            .LabelForward("async_reset", out var async_reset)
            .BPL(async_reset) // system reset detected
            .LDX_Imm(0x10)
            .STX(0x1800) // release data

            .BIT(0x1800) // wait for atn to be released
            .BMI(-5)

            .STY(0x1800) // get ready to read
            .BIT(0)

            .LDX_Imm(2)

            .BIT(0x1800)
            .LabelForward("got0", out var got0)
            .BEQ(got0)

            .SEC()

            .Label(got0)
            .STX(0x1800) // pull data

            .BIT(0x1800) // wait for atn to be pulled
            .BPL(-5)

            .ROL(temp)
            .BCC(bitloop)

            // At this point, the host returns from the seek call.

            .LabelForward("seektrack", out var seektrack)
            .LabelForward("seeksector", out var seeksector)
            .LabelForward("seek_fetchret", out var seek_fetchret)

            .LDY(temp)
            .LDA(seektrack, Y)
            .STA(req_track)
            .LDX(seeksector, Y)
            .INC(interested, X)
            .INC(ninterested)

            .LDA_Imm(0x0c)
            .STA(ledmask)
            .LDA_Imm(seek_fetchret.LowByte())
            .STA(mod_fetchret + 1)
            .LDA_Imm(seek_fetchret.HighByte())
            .STA(mod_fetchret + 2)
            .JMP(drivecode_fetch)

            .Label(async_reset)
            .JMP(_[0xfffc])

            .Label(seek_fetchret)
            // We have the desired continuation record on the stack.
            // Cut off any further units in the same sector.
            .LDA_Imm(0x40)
            .STA(0x100) // no 255-byte unit, cont rec
            .LDA_Imm(0)
            .STA(0x104) // no further units

            // Clear the new-job flag (msb of $103).
            //
            // Also, if it was already clear, this label was
            // targetting the first job. In that case, we should
            // change from track 18 to track 1.

            .LabelForward("notfirst", out var notfirst)
            .ASL(0x103)
            .BCS(notfirst)

            .LDA_Imm(1 * 2)
            .STA(req_track)

            .Label(notfirst)
            .LSR(0x103)

            .LDA_Imm(fetch_return.LowByte())
            .STA(mod_fetchret + 1)
            .LDA_Imm(fetch_return.HighByte())
            .STA(mod_fetchret + 2)
            .JMP(transfer)

            // When we transfer, the continuation record is copied
            // into the continuation buffer along with a do-nothing
            // unit. Then, because there are no more interesting sectors,
            // the continuation buffer is unpacked and the do-nothing
            // unit is transferred.
            // Then we will go and prefetch some interesting sectors.

            .AppendBytes(0x80 - asm.CurrentOffset, 0xff)

            .Label(seektrack).AppendBytes(64, 0)
            .Label(seeksector).AppendBytes(64, 0);
    }
}
