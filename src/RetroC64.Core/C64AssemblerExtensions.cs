...
public static class C64AssemblerExtensions
{
    ...
    /// <param name="defaultFlags">The CPU port flags to set, default is <see cref="CPUPortFlags.Default"/>.</param>
    /// <returns>The assembler instance for chaining.</returns>
    /// <remarks>
    /// Modifies the A register.
    /// </remarks>
    public static C64Assembler SetupRamAccess(this C64Assembler asm, CPUPortFlags defaultFlags = CPUPortFlags.Default, [CallerFilePath] string debugFilePath = "", [CallerLineNumber] int debugLineNumber = 0)
        => asm
            .BeginFunction(debugFilePath, debugLineNumber)
